export class BufferManager {
  ...
  createVertexBuffer() {
    if (!this.context.modelData?.vertexData) {
      throw new Error(
        "Vertex buffer creation failed: No vertex data available"
      );
    }
    ...
    new Float32Array(this.context.vertexBuffer.getMappedRange()).set(
      this.context.modelData.vertexData
    );
  ...
  createIndexBuffer() {
    ...
    new Uint32Array(this.context.indexBuffer.getMappedRange()).set(
      this.context.modelData.indexData
    );
