...
export class OBJLoader {
  ...
  static parseGeometry(lines, state) {
    for (const line of lines) {
      ...
      switch (parts[0]) {
        case "v":
          this.parseVertex(parts, state);
          break;
        case "vt":
          this.parseTexCoord(parts, state);
          break;
        case "vn":
          this.parseNormal(parts, state);
          break;
        case "f":
          this.parseFace(parts, state);
          break;
      }
    }
  }
  ...
  static calculateVertexNormals(state) {
    ...
    for (const triangle of state.faceTriangles) {
      const [v0, v1, v2] = triangle.map((t) =>
        this.parseIndex(t[0], state.positions)
      );
  ...
  static buildVertexBuffer(state) {
    ...
    for (const triangle of state.faceTriangles) {
      for (const vertexDef of triangle) {
        ...
        const texIndex = texIdx
          ? this.parseIndex(texIdx, state.texcoords)
          : null;
        const normIndex = normIdx
          ? this.parseIndex(normIdx, state.normals)
          : posIndex;
        ...
        if (!state.indexMap.has(key)) {
          ...
          const texCoord =
            texIndex !== null ? state.texcoords[texIndex] : [0, 0];
  ...
  static calculateBoundingRadius(bounds) {
    const diagonal = bounds.max.reduce(
      (sum, val, i) => sum + (val - bounds.min[i]) ** 2,
      0
    );
